...
export function getSheetsClient(): sheets_v4.Sheets {
  ...
  const privateKey = base64Key
   ...
   console.error('DEBUG email length:', email.length, JSON.stringify(email));
  console.error('DEBUG key length:', privateKey.length, 'head:', JSON.stringify(privateKey.slice(0, 35)), 'tail:', JSON.stringify(privateKey.slice(-35)));
  console.error('DEBUG key contains CRLF:', privateKey.includes('\r'));
